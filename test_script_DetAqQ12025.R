...
plot.airquality <- function(monitor_name, df24_xy) {
  ...
  p <- ggplot(df24_monitor, aes(x = date, y = PM2.5)) +
    geom_rect(data = aqi_levels_ALL,
              ...
              xmin = xmin, xmax = xmax,
              inherit.aes = FALSE, alpha = 0.2) +
    scale_fill_manual(values = setNames(aqi_levels_ALL$fill, aqi_levels_ALL$category)) +
    geom_line(color = "turquoise4") +
    theme_minimal() + 
    labs(
      x = "", y = "PM 2.5 AQI",
      title = paste("AQI for PM 2.5 (Q1-4 AQI 2024) -", monitor_name)
    ) +
    theme(
...
plot.airquality("3rd Avenue & West Euclid Street", df24_xy)
plot.airquality("Adams Butzel Complex", df24_xy)
...
# Without max min labeled
# PM2.5 boxplot with AQI background
ggplot(df24_xy, aes(x = Name, y = PM2.5)) +
  # Background AQI bands
  geom_rect(
    data = aqi_levels_ALL,
    aes(
...
poll24 <- c("NO2",  "SO2",  "PM1",  "PM2.5", "PM10", "O3",   "CO",  "NO")
...
for (poll24 in aqi_cols24) {
  ...
  # Save PNG
  ggsave(
    filename = paste0(gsub("\\.", "_", poll24), "_24_boxplot.png"),
    plot = p,
    width = 8,
    height = 6,
    dpi = 300
